...
class DC(threading.Thread):
    ...
    def ImageAcquisition(self):
        ...
        for i in range(int(triggerTimeout/100 * frame_cnt * 2)):
            ...
            # add 20240429 test with UJ
            
            if byte > 0 and TT_cnt == 0 :
                TTstt = ti.time()
                self.log.send(self._iam, DEBUG, f'byte = {byte}, TTs = {TTstt - (self.obs_startT.timestamp() - 36000.0)}') 
                sciExpSts = TTstt - (1.45479*byte/8388608.0)
                sciExpSdt = datetime.datetime.fromtimestamp(sciExpSts + 36000.0)
                TT_cnt = 1
        ...
        self.obs_startT = sciExpSdt
        self.obs_endT = datetime.datetime.utcnow()
        #self.log.send(self._iam, DEBUG, f'TTelp = {(self.obs_endT.timestamp() - 36000.0)- TTstt}')
        self.log.send(self._iam, DEBUG, f'TTelp = {(self.obs_endT.timestamp() - 36000.0)- sciExpSts}')
        self.log.send(self._iam, DEBUG, f'TTelp(a-b) = {(self.obs_endT.timestamp() - 36000.0)- (sciExpSts+self.fowlerTime)}')
